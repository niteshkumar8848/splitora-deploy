import { Link, useLocation } from 'react-router-dom';
import { Menu, User, LogOut, Wallet, Building2 } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { Button } from './ui/Button';
import { useState } from 'react';

function Navbar() {
  const location = useLocation();
  const { user, logout } = useAuth();
  const [mobileOpen, setMobileOpen] = useState(false);

  return (
    <nav className="glass-card border-b-0 backdrop-blur-md sticky top-0 z-50 shadow-soft">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="hidden md:flex items-center justify-between border-b border-border/60 py-2">
          <p className="text-xs tracking-[0.14em] uppercase text-muted-foreground">Urban Ledger Workspace</p>
          <p className="text-xs text-muted-foreground">FinOps Grade Group Settlements</p>
        </div>
        <div className="flex justify-between items-center py-4 md:py-3">
          <Link to="/dashboard" className="group flex items-center gap-3">
            <span className="h-10 w-10 rounded-xl bg-gradient-to-br from-primary to-primary-700 text-white inline-flex items-center justify-center shadow-soft">
              <Wallet className="h-5 w-5" />
            </span>
            <div>
              <h1 className="text-2xl md:text-3xl font-bold bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent group-hover:scale-105 transition-transform">
              Splitora
              </h1>
              <p className="hidden md:block text-xs text-muted-foreground -mt-1">Expense Intelligence Platform</p>
            </div>
          </Link>

          <div className="hidden md:flex items-center gap-3">
            {location.pathname !== '/dashboard' && (
              <Link to="/dashboard" className="btn-secondary">
                Dashboard
              </Link>
            )}
            <span className="text-sm text-muted-foreground font-medium px-3 inline-flex items-center gap-1.5">
              <Building2 className="h-4 w-4" />
              {user?.name}
            </span>
            <Button variant="secondary" onClick={logout} className="gap-1.5">
              <LogOut className="h-4 w-4" />
              Logout
            </Button>
          </div>

          <Button variant="ghost" size="icon" className="md:hidden" onClick={() => setMobileOpen(!mobileOpen)}>
            <Menu className="h-6 w-6" />
          </Button>
        </div>

        {mobileOpen && (
          <div className="md:hidden pb-4 animate-slide-up">
            <div className="flex flex-col gap-2 px-2">
              {location.pathname !== '/dashboard' && (
                <Link 
                  to="/dashboard" 
                  className="btn-secondary w-full justify-start"
                  onClick={() => setMobileOpen(false)}
                >
                  Dashboard
                </Link>
              )}
              <Button 
                variant="secondary" 
                className="w-full justify-start gap-2"
                onClick={() => {
                  logout();
                  setMobileOpen(false);
                }}
              >
                <User className="h-4 w-4" />
                {user?.name}
                <LogOut className="h-4 w-4 ml-auto" />
              </Button>
            </div>
          </div>
        )}
      </div>
    </nav>
  );
}

export default Navbar;
